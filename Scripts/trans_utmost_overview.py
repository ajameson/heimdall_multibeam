...
class Classifier(object):
    ...
    def is_hidden(self, cand):
        return ( self.is_noise(cand) |
                 (cand['snr'] < self.snr_cut) |
    ...
    def is_coinc_rfi(self, cand):
        return cand['nbeams'] > self.nbeams_cut
...
class TimeDMPlot(object):
    def __init__(self, g, multiplot):
        ...
        #self.plot_coinc = False
        self.plot_coinc = True
    ...
    def plot(self, data):
        ...
        if (self.plot_coinc and len(data['coinc']['snr']) > 0):
...
if __name__ == "__main__":
    ...
    all_cands = \
        np.loadtxt(filename,
                   dtype={'names': ('snr','samp_idx','time','filter',
                                    ...
                                    'nbeams','prim_beam','max_snr','beam'),
                          'formats': ('f4', 'i4', 'f4', 'i4',
                                      'i4', 'f4', 'i4', 'i4', 'i4',
                                      'i4', 'i4', 'i4', 'f4', 'i4')},
                   skiprows=skip_rows)

    # Adjust for 0-based indexing - dont need this as not masking
    ...
    if not no_plot:    
      ...
      if not interactive:
        g('set terminal png enhanced size ' + res_x + ', ' + res_y)
        if std_out:
          g('set output')
          if verbose:
